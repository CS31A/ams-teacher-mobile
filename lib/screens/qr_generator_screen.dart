...
class _QrGeneratorScreenState extends State<QrGeneratorScreen> {
  ...
  String _qrPayload() {
    ...
    final String date = _fmtDate(_selectedDate);
    final String time = [
      if (_startTime != null) _fmtTime(_startTime),
      if (_startTime != null || _endTime != null) ' - ',
      if (_endTime != null) _fmtTime(_endTime),
    ].join();
    final String room = _selectedRoom == null ? '' : 'ROOM $_selectedRoom';

    // Human-readable block that matches the requested format
    // SUBJECT:\nDATE:\nTIME:\nROOM:
                ...
                child: Column(
                  children: [
                    ...
                    LayoutBuilder(
                      builder: (context, constraints) {
                        final bool narrow = constraints.maxWidth < 520;
                        final Widget startBtn = OutlinedButton.icon(
                          onPressed: () => _pickTime(isStart: true),
                          icon: const Icon(Icons.schedule),
                          label: Text(_startTime == null ? 'Start time' : _fmtTime(_startTime)),
                        );
                        final Widget endBtn = OutlinedButton.icon(
                          onPressed: () => _pickTime(isStart: false),
                          icon: const Icon(Icons.schedule_outlined),
                          label: Text(_endTime == null ? 'End time' : _fmtTime(_endTime)),
                        );
                        if (narrow) {
                          return Column(children: [startBtn, const SizedBox(height: 12), endBtn]);
                        }
                        return Row(children: [Expanded(child: startBtn), const SizedBox(width: 12), Expanded(child: endBtn)]);
                      },
                    ),
                  ],
                ),
